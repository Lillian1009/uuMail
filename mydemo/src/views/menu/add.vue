  ...
  <div>
    ...
    <el-dialog :title="addInfo.isAdd ? '添加菜单' : '编辑菜单'" :visible.sync="addInfo.isShow" center :before-close="cancel">
      <el-form :model="form" :rules="rules" ref="formDialoag">
        ...
        <el-form-item label="菜单地址" :label-width="formLabelWidth" v-else>
          <el-select v-model="form.url" placeholder="请选择">
            <!-- 下拉框循环菜单地址 -->
            <el-option v-for="item in indexRoutes" :key="item.path" :label="item.name" :value="item.path">
            </el-option>
          </el-select>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="addInfo.isAdd" @click="add('formDialoag')" type="primary">添加</el-button>
        <el-button v-else @click="update('formDialoag')" type="primary">编辑</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    reset () {
      this.form = {
        ...
        console.log(this.$refs["formDialoag"]);
      this.$refs["formDialoag"].resetFields();
    },
    //取消事件，关闭弹框，修改父组件数据
    cancel () {
